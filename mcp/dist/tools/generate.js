...
import { getWorkspaceContext, requireClayFile, resolvePath, } from '../shared/workspace-manager.js';
import { executeClayCommand, readClayFile, parseGenerateOutput, } from '../shared/clay-wrapper.js';
export async function generateTool(args) {
    // Validate input
    ...
    try {
        ...
        if (input.model_path && input.output_path) {
            ...
            const result = executeClayCommand('generate', [modelPath, outputPath], workingDir);
            if (!result.success) {
                return {
                    content: [
                        {
